...
public class UserServices implements IService<User> {
   ...
   /* @Override
    public void updateUserNewPass(User user) throws SQLException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, BadPaddingException, InvalidKeyException {
        String req = "UPDATE `user` SET mdp=?/*, urlImage =?*/ /*,`etatUser`=?*//* WHERE addEmail= ?";
      //  PreparedStatement ps = cnx.prepareStatement(req);
        //ps.setInt(1, user.getCin());
     //   ps.setString(1, user.getNom());
      ...
      //  ps.setString(1, user.getMdp());
       // ps.setString(5,user.getUrlImage());
        // ps.setString(4, user.getUrlImage());
       // ps.setString(4, user.getRoleUser().toString());
    //    ps.setString(2, user.getAddEmail());

      //  ps.executeUpdate();
      //  System.out.println("l'Utlisateur est modifié");
   /// }*/


    public static boolean updateUserNewPass(String email, String newPassword) {
        try {
            Connection conn = MyDB.getInstance().getCnx();
            String query = "UPDATE user SET mdp=? WHERE addEmail=?";
            PreparedStatement pstmt = conn.prepareStatement(query);
    ...
    @Override
    public void updateUserByCin(User user) throws SQLException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, BadPaddingException, InvalidKeyException {
        String req = "UPDATE `user` SET nom=? , prenom=?, username = ?, numTlfn = ?, addEmail = ?, mdp = ?, roleUser = ?, confirmationCode = ? WHERE cin = ?";
        PreparedStatement ps = cnx.prepareStatement(req);

        ps.setString(1, user.getNom());
        ps.setString(2, user.getPrenom());
        ps.setString(3, user.getUsername());
        ps.setInt(4, user.getNumTlfn());
        ps.setString(5, user.getAddEmail());
        ps.setString(6, user.getMdp());
      //  ps.setString(5, user.getUrlImage());
        ps.setString(7, user.getRoleUser().toString());
        ps.setString(8, user.getConfirmationCode());
        ps.setInt(9, user.getCin());

        ps.executeUpdate();
        System.out.println("L'utilisateur avec CIN " + user.getCin() + " a été mis à jour.");
    }
